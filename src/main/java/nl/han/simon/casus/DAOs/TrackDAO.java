...
public class TrackDAO {
    ...
    public List<TrackDTO> getPlaylistTracks(int playlistId) {
        var trackResult = queryHelper.executeSelectQuery("SELECT * FROM track\n" +
                "WHERE id IN\n" +
    ...
    public TrackWrapperDTO getAllTracksExcept(int playlistId) {
        ...
        var trackResult = queryHelper.executeSelectQuery("SELECT * FROM [track] WHERE [id] NOT IN (SELECT [trackId] FROM [playlistTracks] WHERE [playlistId] = ?)", getTrackMapper(), playlistId);
    ...
    public TrackWrapperDTO getAllTracks() {
        ...
        var trackResult = queryHelper.executeSelectQuery("SELECT * FROM [track]", getTrackMapper());
    ...
    public void deleteTrackFromPlaylist(int trackId, int playlistId) {
        queryHelper.execute("DELETE FROM [playlistTracks] WHERE [trackId] = ? AND [playlistId] = ?", trackId, playlistId);
    }
